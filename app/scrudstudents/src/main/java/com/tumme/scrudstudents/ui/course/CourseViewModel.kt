...
sealed class CourseEvent {
    ...
    data class ShowMessage(val messageId: Int) : CourseEvent()
}
...
class CourseViewModel @Inject constructor(
     ...
     */
    private val _events = MutableSharedFlow<CourseEvent>()
    ...
    fun deleteCourse(course: CourseEntity) = viewModelScope.launch {
        ...
        _events.emit(CourseEvent.ShowMessage(R.string.course_deleted))
    }
        ...
        fun insertCourse(course: CourseEntity) = viewModelScope.launch {
            ...
            if (course.ectsCourse <= 0) {
                _events.emit(CourseEvent.ShowMessage(R.string.error_ects_must_be_positive))
                return@launch
            }
            ...
            if (course.levelCourse !in validLevels) {
                _events.emit(CourseEvent.ShowMessage(R.string.error_invalid_level))
                return@launch
            }
            ...
            _events.emit(CourseEvent.SaveSuccess)
        }
